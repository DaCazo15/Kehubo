...
export async function detectCountryFromIP() {
  if (typeof window !== 'undefined') {
    const cached = localStorage.getItem('kehubo_user_country')
    if (cached) return cached.toUpperCase()
  }

  // 1. Intentar con country.is (rápido, sin límites de tasa)
  try {
    ...
    if (res.ok) {
      ...
      if (data.country) {
        ...
        if (typeof window !== 'undefined') localStorage.setItem('kehubo_user_country', c)
        return c
      }
    }
  } catch (e) {}
  ...
  // 3. Intentar con BigDataCloud
  try {
    const res = await fetch('https://api.bigdatacloud.net/data/client-info')
    if (res.ok) {
      const data = await res.json()
      if (data.countryCode) {
