...
<template>
  <Teleport to="body">
    ...
    >
      ...
      <div 
        @click="emit('close')"
        class="fixed inset-0 bg-slate-950/80 backdrop-blur-md transition-opacity animate-fadeIn"
      ></div>

      <!-- Contenedor del Modal -->
      <div 
        class="relative w-full max-w-2xl bg-slate-900/95 border border-amber-500/40 rounded-3xl p-6 sm:p-8 space-y-6 shadow-2xl z-10 my-8 animate-fadeIn max-h-[90vh] overflow-y-auto"
      >
        <!-- Cabecera del Modal -->
        <div class="flex items-center justify-between pb-4 border-b border-slate-800">
          <div>
            <h3 class="text-xl font-black uppercase text-slate-100 tracking-wide flex items-center gap-2">
          ...
          <button
            type="button"
            @click="emit('close')"
            class="p-2 rounded-xl text-slate-400 hover:text-slate-100 hover:bg-slate-800 transition"
          >
        ...
        <!-- Footer / Botón Guardar Cambios -->
        <div class="pt-4 border-t border-slate-800 flex justify-end gap-3">
          <button
            type="button"
            @click="emit('close')"
            class="py-2.5 px-5 rounded-xl border border-slate-700 text-slate-300 hover:bg-slate-800 font-bold text-xs uppercase tracking-wider transition"
          >
            Cancelar
          </button>
          <button
            ...
            @click="handleSave"
            :disabled="saving"
            class="game-btn-gold py-2.5 px-6 rounded-xl text-slate-950 font-black text-xs uppercase tracking-wider flex items-center gap-2 disabled:opacity-50"
          >
            <svg 
              v-if="saving" 
